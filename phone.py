...
try:
    ...
    print ("By Soydaş (Denemedir Paylaşmayın...)" ),
    print("Gerekli modüller indiriliyor...(Phone version Sadece androidlerde calısır)")
print(
        "Hata alırsanız `pip3 install -r requirements.txt` "
        " yazarak tüm paketleri indirin")
    ...
    while True:
        try:
            number = int(input(f"""Telefon numarasını yazın. Şunun gibi: "54xxxxxxxx" (Sadece Türkiye numaralarında çalışır! --
                               ---- 
                               --- -
                               (Döngüye aldıktan sonra Komple programı kapatmanız gerekir.))\n[?] : """))
...
def get_worker_amount():
    ...
    while True:
        try:
            worker_amount = int(input(f"Thread(konu) sayısını girin. Tavsiye edilen 40-100 arasıdır.\n[?] : "))
            if worker_amount >= 1:
